...
import { getTeamMembers, createTeamMember, updateTeamMember, deleteTeamMember, TeamMember, translateText, uploadTeamImage, updateTeamMemberOrders } from '../../services/apiService';
...
const TeamMemberCard: React.FC<{
  ...
  isAutoGeneratedAvatar: (url: string) => boolean;
  isDragging?: boolean;
}> = ({ member, isAdmin, canEdit, onEdit, onDelete, deletingId, isAutoGeneratedAvatar, isDragging }) => {
  const [imageError, setImageError] = useState(false);
  const [isGeneratingAvatar, setIsGeneratingAvatar] = useState(false);

  // Reset image error when member.image_url changes
  useEffect(() => {
    setImageError(false);
    // If there's no image_url, assume we're generating an avatar
    if (!member.image_url) {
      setIsGeneratingAvatar(true);
      // Try to load the member data again after a delay
      const timer = setTimeout(() => {
        setIsGeneratingAvatar(false);
      }, 3000); // Show loading state for 3 seconds
  ...
  return (
    <div 
      className={`bg-white overflow-hidden shadow rounded-lg border transition-all duration-200 ${
        isDragging ? 'border-indigo-500 shadow-lg ring-2 ring-indigo-500 opacity-50' : 'border-gray-200'
      }`}
    >
      <div className="p-5">
          ...
          <div className="flex space-x-2">
            {canEdit && (
              <>
                ...
                {isAdmin && (
                  <button
                    onClick={() => onDelete(member.team_id)}
...
const SortableTeamMember: React.FC<{
  member: TeamMember;
  isAdmin: boolean;
  onEdit: (member: TeamMember) => void;
  onDelete: (id: number) => void;
  deletingId: number | null;
  isAutoGeneratedAvatar: (url: string) => boolean;
  hasEditPermission: (member: TeamMember) => boolean;
}> = (props) => {
  const {
    attributes,
    listeners,
    ...
    transition,
    isDragging,
  } = useSortable({ id: props.member.team_id });

  const style = {
    transform: CSS.Transform.toString(transform),
    transition,
    opacity: isDragging ? 0 : 1,
    cursor: props.isAdmin ? 'move' : 'default',
  };

  return (
    <div ref={setNodeRef} style={style} {...(props.isAdmin ? { ...attributes, ...listeners } : {})}>
      <TeamMemberCard 
        {...props} 
        isDragging={isDragging}
        canEdit={props.hasEditPermission(props.member)}
      />
    </div>
  );
};
...
const TeamPage: React.FC = () => {
  ...
  const handleSaveMember = async () => {
    try {
      ...
      const memberData = {
        ...
        // If image_url is empty string or undefined, send null to trigger avatar generation
        image_url: formData.image_url || null
      };
      
      let savedMemberId: number | undefined;
      let savedMember: TeamMember | undefined;
      if (selectedMember) {
        // Update existing member
        const result = await updateTeamMember(selectedMember.team_id, memberData);
        if (result.success) {
          savedMemberId = selectedMember.team_id;
          // Create a temporary member object with existing timestamps
          savedMember = {
            ...selectedMember,
            ...memberData,
        ...
        if (result.success) {
          ...
          // Create a temporary member object with current timestamps
      ...
      if (savedMemberId && savedMember) {
        // Close the modal first
        setIsModalOpen(false);
        
        // Immediately update the UI with the temporary member object
        const currentMembers = [...members];
        const memberIndex = currentMembers.findIndex(m => m.team_id === savedMemberId);
        
        if (memberIndex !== -1) {
          // Update existing member
          currentMembers[memberIndex] = savedMember;
        } else {
          // Add new member
          currentMembers.push(savedMember);
        }
        
        // Sort and update the UI immediately
        const sortedMembers = [...currentMembers].sort((a, b) => a.display_order - b.display_order);
        setMembers(sortedMembers);

        // Function to fetch and update members
        const fetchAndUpdateMembers = async () => {
          const updatedMembers = await getTeamMembers();
          const refreshedMember = updatedMembers.find(m => m.team_id === savedMemberId);
          if (refreshedMember) {
            // Update the member with the fresh data
            const freshMembers = members.map(m => 
              m.team_id === savedMemberId ? refreshedMember : m
            );
            // Sort members by display_order
            const sortedMembers = [...freshMembers].sort((a, b) => a.display_order - b.display_order);
            setMembers(sortedMembers);
            return true;
          }
          return false;
        ...
        // Try to fetch the updated data multiple times
        let attempts = 0;
        const maxAttempts = 3;
        ...
        while (attempts < maxAttempts) {
          ...
          const success = await fetchAndUpdateMembers();
          if (success) break;
          attempts++;
        }

        // Final attempt if all retries failed
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    const draggedMember = members.find(m => m.team_id === active.id);
    if (draggedMember) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (!over || active.id === over.id || !hasAdminPermission()) return;
    
    const oldIndex = members.findIndex(m => m.team_id === active.id);
    const newIndex = members.findIndex(m => m.team_id === over.id);
    
    const reorderedMembers = arrayMove(members, oldIndex, newIndex);
    const updatedMembers = reorderedMembers.map((member: TeamMember, index: number) => ({
      ...member,
    ...
    try {
      const result = await updateTeamMemberOrders(updatedMembers.map(member => ({
        team_id: member.team_id,
        display_order: member.display_order
      })));
      
      if (!result.success) {
        console.error('Failed to update team member orders');
        // Fetch fresh data from server on failure
        const refreshedData = await getTeamMembers();
        const sortedMembers = [...refreshedData].sort((a, b) => a.display_order - b.display_order);
        setMembers(sortedMembers);
      }
    } catch (error: any) {
      ...
      try {
        ...
        setError('Failed to refresh team members. Please reload the page.');
      }
    }
  };
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {members.map((member) => (
                <SortableTeamMember
                  ...
                  isAutoGeneratedAvatar={isAutoGeneratedAvatar}
          ...
          <DragOverlay>
            {activeMember ? (
              <TeamMemberCard
                member={activeMember}
                isAdmin={hasAdminPermission()}
                canEdit={hasEditPermission(activeMember)}
                onEdit={handleEditMember}
                onDelete={handleDeleteMember}
                deletingId={deletingId}
                isAutoGeneratedAvatar={isAutoGeneratedAvatar}
                      ...
                      <div className="mb-4">
                        <div className="mx-auto h-32 w-32 rounded-full overflow-hidden bg-gray-100">
                          <img 
                            ...
                            className="h-full w-full object-cover"
                            onError={(e) => {
                              // If the image fails to load, clear the image_url to trigger avatar generation
